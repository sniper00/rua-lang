...
fn run(args: &[String]) -> Result<(), String> {
    ...
    match cmd {
        ...
        Some("metadata") => metadata(&args[2..]),
        Some("-h") | Some("--help") | None => {
            print_usage();
            Ok(())
        }
        Some(other) => Err(format!(
            "unknown command `{}` (try `build`, `check`, `metadata`, or `--help`)",
            other
        )),
    }
}
...
const GENERATED_ANNOTATION_HEADER: &str =
    "-- Generated by ruac. Runtime annotation metadata ABI 1.\n";

fn remove_stale_annotation_outputs(directory: &Path) -> Result<(), String> {
    if !directory.exists() {
        return Ok(());
    }
        ...
        .map_err(|error| format!("reading {}: {error}", directory.display()))?
    {
        ...
        if file_type.is_dir() {
            remove_stale_annotation_outputs(&entry.path())?;
            continue;
        }
        ...
        if file_name != "rua_annotations.lua" && !file_name.ends_with(".meta.lua") {
            continue;
        }
        let path = entry.path();
        let generated = std::fs::read_to_string(&path)
            .map(|source| source.starts_with(GENERATED_ANNOTATION_HEADER))
...
fn metadata(args: &[String]) -> Result<(), String> {
    let args = parse_command_args(args, "metadata", false)?;
    let options = load_compile_options(&args)?;
    let metadata =
        ruac::compile_path_metadata_with_options(&args.input, &options, args.annotation.as_deref())
            .map_err(|error| error.to_string())?;
    print!("{metadata}");
    Ok(())
}
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--features" => {
                ...
                let configured = args
                    .get(i)
                    .ok_or("`--features` requires a comma-separated list")?;
                for feature in configured.split(',') {
                    if feature.is_empty() {
                        return Err("`--features` contains an empty feature name".to_string());
                    }
            ...
            "--cfg" => {
                ...
                let configured = args
                    .get(i)
                    .ok_or("`--cfg` requires `flag` or `key=value`")?;
                if configured.is_empty() {
                    return Err("`--cfg` cannot be empty".to_string());
                }
                cfg.push(configured.to_string());
            }
            "--annotation" if command == "metadata" => {
                i += 1;
                let path = args
                    .get(i)
                    .ok_or("metadata: `--annotation` requires a qualified name")?;
                annotation = Some(path.to_string());
            }
            "--format" if command == "metadata" => {
                i += 1;
                match args.get(i).map(String::as_str) {
                    Some("toml") => {}
                    Some(format) => {
                        return Err(format!(
                            "metadata: unsupported format `{format}` (expected `toml`)"
                        ));
                    }
                    None => return Err("metadata: `--format` requires `toml`".to_string()),
                }
            }
            other => {
...
fn load_compile_options(command: &CommandOptions) -> Result<ruac::CompileOptions, String> {
    ...
    for feature in &command.features {
        options.cfg.insert_feature(feature);
    }
    for configured in &command.cfg {
        if let Some((key, value)) = configured.split_once('=') {
            if key.is_empty() || value.is_empty() {
                return Err(format!(
                    "invalid --cfg `{configured}`; expected `flag` or `key=value`"
                ));
            }
            options.cfg.insert_value(key, value.trim_matches('"'));
        } else {
            options.cfg.insert_flag(configured);
        }
    }
    Ok(options)
}
