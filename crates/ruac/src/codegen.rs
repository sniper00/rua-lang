...
pub fn generate_with_source_map(
    ...
    cg.register_declaration_imports();
...
impl Codegen<'_> {
    fn register_declaration_imports(&mut self) {
        let modules = self
            .hir
            .modules
            .iter()
            .filter(|module| {
                module.is_declaration
                    && module
                        .parent
                        .is_some_and(|parent| !self.hir.module(parent).is_declaration)
                    && module.path.segments().first().map(String::as_str) != Some("__rua_builtin")
            })
        ...
        for module in modules {
            ...
            let parent = data.parent.expect("declaration module has a parent");
            let alias = if parent == self.hir.root {
                let alias = self
                    .layout
                    .module(module)
                    .expect("root declaration module has a backend place")
                    .to_string();
                ...
                let preferred = data
                    ...
                    .map(String::as_str)
                    .unwrap_or("library");
                self.register_runtime_import(&runtime, None, preferred, None)
            };
            self.declaration_imports.insert(module, alias);
        }
    }
    ...
    fn declare_mod(&mut self, m: &ModDecl, module: crate::hir::ModuleId) {
        ...
        for (item_index, item) in m.items.iter().enumerate() {
            if let Item::Mod(child) = item {
                let child_module = self.program.child_module(module, item_index);
                if child.is_decl {
                    if let Some(alias) = self.declaration_imports.get(&child_module) {
                        let place = self
                            .layout
                            .module(child_module)
                            .expect("declaration module has a backend place")
                            .expression();
                        self.assign(place, LuaExpr::name(alias));
                    }
                } else {
            ...
            ExprKind::MethodCall {
                ...
                if let Some(definition) = self.info.standard_method(e.id) {
                    ...
                    match self.hir.language_item(definition) {
                        ...
                        Some(
                            L::ResultUnwrap | L::ResultUnwrapOr | L::ResultIsOk | L::ResultIsErr,
                        ) => {
                            let arguments = args
                                .iter()
                    ...
                    if let Some(runtime) = self.hir.standard_runtime(definition).cloned() {
                        if runtime.dispatch == rua_resources::StdDispatch::Module {
                            ...
                            arguments.extend(args.iter().map(|argument| self.gen_inline(argument)));
