        ...
        analysis
            ...
            .any(|diagnostic| matches!(
                ...
                InferenceDiagnostic::TypeMismatch {
                    source: rua_ide::analysis::InferenceSource::Pattern(_),
                    ..
                }
            ))
    );
...
fn table_mutations_inside_for_are_reported_as_warnings() {
    ...
    assert!(compiler_diagnostics.iter().any(|diagnostic| {
        diagnostic.diagnostic.code == rua_ide::analysis::DiagnosticCode::LintUnsafeTableMutation
    }));

    let (host, file_id) = single_file_host(SOURCE);
    let diagnostics = host.analysis().diagnostics(file_id);
    assert!(diagnostics.iter().any(|diagnostic| {
        diagnostic.code() == Some(rua_ide::analysis::DiagnosticCode::LintUnsafeTableMutation)
            && diagnostic.severity() == rua_ide::analysis::DiagnosticSeverity::Warning
    }));
}
