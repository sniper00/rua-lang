...
fn builtin_members_hover_and_resolve_to_shared_declarations() {
    ...
    for (needle, expected_name, expected_documentation) in [
        ...
        let builtin = include_str!("../../rua-common/resources/std/option.ruai");
...
fn result_type_and_variants_hover_and_resolve_in_expressions_and_patterns() {
    ...
    srv.open(&uri, source);
    let analysis = srv.snapshot();
    let declaration = include_str!("../../rua-common/resources/std/result.ruai");
...
fn hover_shows_qualified_module_type_context_for_type_and_variant() {
    let main_uri = uri("/test/main.rua");
    let core_uri = uri("/test/rua_common.rua");
    let source = concat!(
        "fn inspect(code: rua_common::DiagnosticCode) {\n",
        "    let selected = rua_common::DiagnosticCode::LintUnusedFunction;\n",
        "}\n",
    );
    let mut srv = TestServer::new();
    srv.open(&main_uri, source);
    ...
    for (needle, offset_in_needle, expected_signature) in [
        ...
        assert_eq!(hover.context(), Some("rua_common::DiagnosticCode"));
