...
use rua_ide::analysis::{ProjectFile, ProjectId};
...
fn semantic_tokens_include_variable_and_function_kinds() {
    ...
    let has_variable = kinds
        .iter()
        .any(|k| matches!(k, rua_ide::analysis::SemanticTokenKind::Variable));
    let has_parameter = kinds
        .iter()
        .any(|k| matches!(k, rua_ide::analysis::SemanticTokenKind::Parameter));
    let has_function = kinds
        .iter()
        .any(|k| matches!(k, rua_ide::analysis::SemanticTokenKind::Function));
