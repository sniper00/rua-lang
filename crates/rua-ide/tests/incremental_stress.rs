...
impl TestServer {
    ...
    fn snapshot(&self) -> rua_ide::analysis::Analysis {
        self.host.analysis()
    }
...
fn method_completion_filters_self_from_snippet() {
    ...
    match insert {
        Some(rua_ide::analysis::CompletionInsert::Call { params, .. }) => {
...
fn member_completion_detail_separates_label_and_type() {
    ...
    let x_field = items
        .iter()
        .find(|i| i.label() == "x" && i.kind() == rua_ide::analysis::CompletionKind::Field)
...
fn postfix_completions_after_dot() {
    ...
    if let Some(rua_ide::analysis::CompletionInsert::Snippet(text)) = if_item.insert() {
        assert!(
            text.contains("x"),
...
fn keyword_snippets_include_placeholders() {
    ...
    match for_item.insert() {
        Some(rua_ide::analysis::CompletionInsert::Snippet(_text)) => {
    ...
    assert!(
        !matches!(
            else_item.insert(),
            Some(rua_ide::analysis::CompletionInsert::Snippet(_))
        ),
...
fn member_hover_shows_field_type() {
    ...
    let analysis = srv.snapshot();
    let source = analysis.parse(file_id).syntax_node().text().to_string();
    let li = rua_ide::syntax::LineIndex::new(&source);
    ...
    for definition in def_map.definitions() {
        ...
        for (bid, binding) in body.bindings() {
            if binding.name() == Some("p") {
                ...
                if let rua_ide::analysis::Ty::Named(named) = ty {
...
fn lint_self_not_flagged_as_unused() {
    ...
    let has_w0300 = diags
        .iter()
        .any(|d| d.code() == Some(rua_ide::analysis::DiagnosticCode::LintUnusedVariable));
    assert!(
        !has_w0300,
...
fn lint_unreachable_ignores_keywords_in_identifiers() {
    ...
    let has_w0302 = diags
        .iter()
        .any(|d| d.code() == Some(rua_ide::analysis::DiagnosticCode::LintUnreachableCode));
    assert!(
        !has_w0302,
