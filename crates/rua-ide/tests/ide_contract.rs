...
fn ide_contract_canonical_reexports_are_the_same_types() {
    ...
    let hir_range: rua_ide::analysis::hir::TextRange = range;
    let ide_range: rua_ide::analysis::ide::TextRange = hir_range;
    assert_eq!(ide_range, range);

    let position = FilePosition::new(FileId::new(2), 3);
    let semantic_position: rua_ide::analysis::semantic::FilePosition = position;
    let ide_position: rua_ide::analysis::ide::FilePosition = semantic_position;
...
fn ide_contract_navigation_hover_and_completion_are_protocol_neutral() {
    ...
    assert_eq!(hover.context(), Some("geometry::Point"));
    ...
    assert_eq!(hint.label_parts().len(), 1);
    assert_eq!(hint.label_parts()[0].value(), "Point");
