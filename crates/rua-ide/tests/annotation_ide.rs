...
fn analysis(source: &str) -> (rua_ide::analysis::Analysis, FileId, ProjectId) {
...
fn annotation_completion_is_schema_aware() {
    ...
    assert_eq!(route.kind(), rua_ide::analysis::CompletionKind::Annotation);
    assert_eq!(
        route.detail(),
...
fn annotation_diagnostics_report_schema_argument_errors() {
    ...
    assert!(
        diagnostics.iter().any(|diagnostic| {
            diagnostic.code() == Some(rua_ide::analysis::DiagnosticCode::AnnotationInvalidArguments)
                && diagnostic
                    .message()
