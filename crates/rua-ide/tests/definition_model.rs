...
fn item_signature_records_generics_bounds_receivers_where_and_extern() {
    ...
    let implementation = tree
        .items()
        .iter()
        .find(|item| item.kind() == rua_ide::analysis::ItemKind::Impl)
        .unwrap();
    ...
    let external = tree
        .items()
        .iter()
        .find(|item| item.kind() == rua_ide::analysis::ItemKind::ExternFunction)
        .unwrap();
...
fn module(map: &DefMap, name: &str) -> rua_ide::analysis::ModuleId {
