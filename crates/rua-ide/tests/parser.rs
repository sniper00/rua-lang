...
use rua_ide::syntax::{SyntaxKind, parse_source_file};

fn has_kind(node: &rua_ide::syntax::SyntaxNode, kind: SyntaxKind) -> bool {
    if node.kind() == kind {
        return true;
    }
    for child in node.children_with_tokens() {
        if let rua_ide::syntax::SyntaxElement::Node(n) = child
            && has_kind(&n, kind)
        {
            return true;
        }
...
fn count_kind(node: &rua_ide::syntax::SyntaxNode, kind: SyntaxKind) -> usize {
...
fn parse_and_check(source: &str) -> rua_ide::syntax::SyntaxNode {
...
fn parse_no_errors(source: &str) -> rua_ide::syntax::SyntaxNode {
    let parse = parse_source_file(source);
...
fn legacy_mod_declarations_are_rejected() {
    let parsed = rua_ide::syntax::parse_source_file("mod math;");
...
fn parse_top_level_chunk_statements_between_items() {
    let source =
        "let before = 1;\nfn value() -> i64 { 2 }\nprint(\"{}\", before);\nlet after = value();";
    let parsed = rua_ide::syntax::parse_source_file(source);
