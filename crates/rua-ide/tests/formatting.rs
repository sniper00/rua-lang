...
fn formatting_produces_output() {
    ...
    let formatted = rua_ide::syntax::format::format_str(&source);
...
fn formatting_is_idempotent() {
    ...
    let first = rua_ide::syntax::format::format_str(&source);
    let second = rua_ide::syntax::format::format_str(&first);
