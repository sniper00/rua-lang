...
pub(super) struct LibraryConfig {
    ...
    pub(super) project_cfg: BTreeMap<u32, rua_common::CfgOptions>,
    pub(super) files: Vec<LibraryFile>,
    pub(super) std_root: Option<PathBuf>,
    pub(super) standard_library: Option<rua_common::StdLibrary>,
}
...
pub(super) fn merge_project_configs(
    ...
    for (project_index, root) in workspace_roots.iter().enumerate() {
        let config_path = root.join(rua_common::PROJECT_CONFIG_FILE);
...
impl LibraryScanRequest {
    ...
    pub(super) fn scan(
        ...
        let standard_library = self
            .std_root
            .as_deref()
            .map(rua_common::load_std_dir)
            .transpose()
            .map_err(|error| error.to_string())?;
        if let Some(library) = &standard_library {
            crate::analysis::hir::StdLibraryIndex::build(library)
...
fn parse_cfg_options(setting: &serde_json::Value) -> Result<rua_common::CfgOptions, String> {
    let mut cfg = rua_common::CfgOptions::default();
...
mod tests {
    ...
    fn std_path_loads_a_manifest_driven_library() {
        let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../rua-common/resources/std");
    ...
    fn ruarc_toml_supplies_project_libraries_and_editor_settings_override() {
        ...
        std::fs::write(
            root.join(rua_common::PROJECT_CONFIG_FILE),
            r#"
            [workspace]
