//! The Rua Language Server, communicating via stdio JSON-RPC (LSP).
//!
//! All semantic queries go through a single long-lived [`AnalysisHost`];
//! there is no legacy workspace or compiler bridge in the production path.
...
use crate::lsp::state::*;
use crate::lsp::worker::WorkerPool;
...
impl Server {
                    ...
                    .filter_map(|r| {
                        ...
                        self.range_for_file(file_range.file_id, file_range.range)
                            .map(|range| DocumentHighlight {
                                range,
                                kind: match r.kind() {
                                    crate::analysis::ReferenceKind::Write => {
                                        Some(DocumentHighlightKind::WRITE)
                                    }
                                    crate::analysis::ReferenceKind::Read => {
                                        Some(DocumentHighlightKind::READ)
                                    }
                                    crate::analysis::ReferenceKind::Declaration => {
    ...
    fn handle_code_action(&mut self, req: Request) {
        ...
        for diag in &params.context.diagnostics {
            if let Some(lsp_types::NumberOrString::String(code_str)) = diag.code.as_ref() {
                match code_str.as_str() {
                    ...
                    code if code
                        == crate::analysis::DiagnosticCode::TypeImmutableAssignment.as_str() =>
                    {
                        let diag_range = &diag.range;
        ...
        for definition in def_map.definitions() {
            ...
            for (expr_id, expr) in body.exprs() {
                let crate::analysis::Expr::Match { scrutinee, arms } = expr else {
                    continue;
                };
                ...
                let Some(enum_template) = (|| {
                    let named = match &scrutinee_ty {
                        crate::analysis::Ty::Named(n) => n,
                        _ => return None,
                    };
                    let d = def_map.definition(named.definition())?;
                    if d.kind() != crate::analysis::DefKind::Enum {
                        return None;
                    }
                ...
                let existing_variants: HashSet<DefId> = arms
                    ...
                    .filter_map(|arm| {
                        ...
                        let path = match pat {
                            crate::analysis::Pat::Path(p)
                            | crate::analysis::Pat::TupleVariant { path: p, .. }
                            | crate::analysis::Pat::StructVariant { path: p, .. } => p,
                            _ => return None,
                        };
                        let last_seg = path.last()?;
                        let range = source_map.name_ref_range(*last_seg)?;
                        semantics
                            .find_def_at(crate::analysis::FilePosition::new(
                                range.file_id,
                                range.range.start(),
                            ))
                ...
                for variant in &all_variants {
                    let crate::analysis::MemberTarget::Definition(variant_id) = variant.target()
                    else {
                        continue;
                    };
                    ...
                    let crate::analysis::ItemSignature::Variant(signature) =
                        variant_definition.signature()
                    else {
                        continue;
                    };
                    let pattern = match signature.kind() {
                        crate::analysis::VariantKind::Unit => name.to_string(),
                        crate::analysis::VariantKind::Tuple => format!(
                            "{name}({})",
                            vec!["_"; signature.tuple_types().len()].join(", ")
                        ),
                        crate::analysis::VariantKind::Struct => {
                            format!("{name} {{ .. }}")
                        }
                    };
    ...
    fn handle_call_hierarchy_incoming(&mut self, req: Request) {
        ...
        let result: Vec<lsp_types::CallHierarchyIncomingCall> = self
            ...
            .map(|file_id| {
                ...
                let chi = crate::analysis::CallHierarchyItem {
                    project_id,
                    target,
                    name: params.item.name.clone(),
                    kind: crate::analysis::DefKind::Function,
                    file_id,
                    range: self
    ...
    fn handle_type_hierarchy_subtypes(&mut self, req: Request) {
        ...
        let result: Vec<lsp_types::TypeHierarchyItem> = self
            ...
            .map(|file_id| {
                ...
                let thi = crate::analysis::TypeHierarchyItem {
                    project_id,
                    target,
                    name: params.item.name.clone(),
                    kind: crate::analysis::DefKind::Trait,
                    file_id,
                    range: self
                        .text_range_for_file(file_id, params.item.selection_range)
                        .unwrap_or_else(|| TextRange::new(0, 0)),
        ...
        let result: Vec<lsp_types::TypeHierarchyItem> = self
            ...
            .map(|file_id| {
                ...
                let thi = crate::analysis::TypeHierarchyItem {
                    ...
                    target,
                    name: params.item.name.clone(),
                    kind: crate::analysis::DefKind::Struct,
                    file_id,
                    range: self
                        .text_range_for_file(file_id, params.item.selection_range)
                        .unwrap_or_else(|| TextRange::new(0, 0)),
    ...
    fn apply_library_config(&mut self, config: LibraryConfig) {
        let standard_library = config
            ...
            .as_ref()
            .map(Ok)
            .unwrap_or_else(|| rua_common::embedded_std().map_err(ToString::to_string));
        ...
        self.standard_library_navigation_root = config.std_root.clone().or_else(|| {
            rua_common::materialized_embedded_std()
                .ok()
    ...
    fn handle_watched_file_change(&mut self, params: &DidChangeWatchedFilesParams) {
        ...
        if standard_library_changed
            ...
            && let Ok(library) = rua_common::load_std_dir(root)
        {
    ...
    fn ref_to_location(&self, r: &crate::analysis::ReferenceResult) -> Option<Location> {
        let file_range = r.range();
        let uri = self.uri_for_file(file_range.file_id)?;
        let range = self.range_for_file(file_range.file_id, file_range.range)?;
        Some(Location { uri, range })
    ...
    fn type_hierarchy_item_to_lsp(
        &self,
        item: &crate::analysis::TypeHierarchyItem,
...
/// Build the shared markdown body: optional backtick-quoted context, code-fenced
/// signature, and optional documentation section. Used by both hover cards and
/// inlay-hint tooltips.
fn render_signature_markdown(
    context: Option<&str>,
    signature: &str,
...
fn to_lsp_type_hint_tooltip(tooltip: &TypeHintTooltip) -> InlayHintLabelPartTooltip {
    let value = render_signature_markdown(
        tooltip.context(),
        tooltip.signature(),
...
fn make_resolve_data(target: Option<crate::analysis::FileRange>) -> Option<serde_json::Value> {
    let target = target?;
...
fn parse_resolve_data(value: &serde_json::Value) -> Option<crate::analysis::FileRange> {
    ...
    Some(crate::analysis::FileRange::new(
        file_id,
...
fn completion_to_lsp(
    item: &crate::analysis::CompletionItem,
    line_index: &LineIndex,
    source: &str,
    _file_id: crate::analysis::FileId,
) -> CompletionItem {
...
fn core_diag_to_lsp(
    diagnostic: &crate::analysis::Diagnostic,
    line_index: &LineIndex,
    source: &str,
) -> Diagnostic {
    ...
    let severity = match diagnostic.code() {
        Some(c) => match c.severity() {
            crate::analysis::DiagnosticSeverity::Error => Some(DiagnosticSeverity::ERROR),
            crate::analysis::DiagnosticSeverity::Warning => Some(DiagnosticSeverity::WARNING),
        },
...
fn encode_semantic_tokens(
    tokens: &[crate::analysis::SemanticToken],
    line_index: &LineIndex,
    source: &str,
...
fn build_document_symbol_tree(
    symbols: &[crate::analysis::DocumentSymbol],
    li: &LineIndex,
    src: &str,
) -> Vec<DocumentSymbol> {
    symbols
...
fn format_edits(src: &str) -> Vec<TextEdit> {
    let formatted = crate::syntax::format::format_str(src);
    if formatted == src {
        return Vec::new();
    }
    Vec::from([TextEdit {
...
mod tests {
    ...
    fn hover_markdown_renders_qualified_type_context_before_signature() {
        let hover = HoverResult::new(
            crate::analysis::FileRange::new(FileId::new(0), crate::analysis::TextRange::new(0, 18)),
            "LintUnusedFunction",
        )
        .with_context("rua_common::DiagnosticCode");
        ...
        assert!(
            content
                .value
                .starts_with("`rua_common::DiagnosticCode`\n\n```rua\nLintUnusedFunction\n```")
        );
    }
    ...
    fn format_edits_produces_whole_document_edit() {
        ...
        assert_eq!(edits[0].new_text, crate::syntax::format::format_str(src));
    }
    ...
    fn semantic_token_encoding_skips_invalid_source_ranges() {
        ...
        let tokens = [
            ...
            crate::analysis::SemanticToken::new(
                crate::analysis::FileRange::new(
                    FileId::new(0),
                    crate::analysis::TextRange::new(100, 101),
                ),
                crate::analysis::SemanticTokenKind::Variable,
                crate::analysis::SemanticTokenModifiers::NONE,
            ),
    ...
    fn sort_text_higher_relevance_sorts_first() {
        ...
        let keyword =
            crate::analysis::CompletionItem::new("fn", crate::analysis::CompletionKind::Keyword)
                ...
                .with_relevance(crate::analysis::CompletionRelevance::keyword());
    ...
    fn label_details_type_like_kinds_have_colon_prefix() {
        ...
        for kind in [
            crate::analysis::CompletionKind::Field,
            crate::analysis::CompletionKind::Variable,
            crate::analysis::CompletionKind::Parameter,
            crate::analysis::CompletionKind::Variant,
        ] {
            let item = crate::analysis::CompletionItem::new("x", kind).with_detail("i64");
            let li = empty_line_index();
            let lsp = completion_to_lsp(&item, &li, "", FileId::new(0));
            let ld = lsp
                .label_details
    ...
    fn label_details_signature_kinds_have_space_prefix() {
        ...
        for kind in [
            crate::analysis::CompletionKind::Function,
            crate::analysis::CompletionKind::Method,
            crate::analysis::CompletionKind::Keyword,
        ] {
            let item = crate::analysis::CompletionItem::new("greet", kind)
