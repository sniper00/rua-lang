...
use crate::syntax::kind::{SyntaxElement, SyntaxKind as K, SyntaxNode, SyntaxToken};
...
impl Attribute {
    ...
    pub fn to_core(&self) -> Result<rua_common::Attribute, String> {
        let item = self
            .meta_item()
...
impl MetaItem {
    ...
    pub fn to_core(&self) -> Result<rua_common::MetaItem, String> {
        ...
        if matches!(
            ...
            return Ok(rua_common::MetaItem::Literal(meta_literal(first)?));
        }
        ...
        if direct_tokens.iter().any(|token| token.kind() == K::Eq) {
            let value = child::<MetaValue>(&self.syntax)
                ...
                .to_core()?;
            return Ok(rua_common::MetaItem::NameValue { name, value });
        }
        let items = self
            .nested()
        ...
        if direct_tokens.iter().any(|token| token.kind() == K::LParen) {
            Ok(rua_common::MetaItem::List { name, items })
        } else {
            Ok(rua_common::MetaItem::Word(name))
        }
    }
}

impl MetaValue {
    pub fn to_core(&self) -> Result<rua_common::MetaValue, String> {
        let direct_tokens = self
            .syntax
            .children_with_tokens()
            .filter_map(|element| element.into_token())
        ...
        if first.kind() == K::Ident {
            let path = direct_tokens
                ...
                .collect::<String>();
            return Ok(rua_common::MetaValue::Path(path));
        }
        meta_literal(first)
    }
}

fn meta_literal(token: &SyntaxToken) -> Result<rua_common::MetaValue, String> {
    match token.kind() {
        K::Str => token
            .text()
            .strip_prefix('"')
            .and_then(|text| text.strip_suffix('"'))
            .map(|text| rua_common::MetaValue::String(text.to_string()))
            .ok_or_else(|| "attribute string must use double quotes".to_string()),
        K::KwTrue => Ok(rua_common::MetaValue::Bool(true)),
        K::KwFalse => Ok(rua_common::MetaValue::Bool(false)),
        K::Int => token
            .text()
            .replace('_', "")
            .parse::<i64>()
            .map(rua_common::MetaValue::Integer)
            .map_err(|_| "attribute integer is outside the i64 range".to_string()),
        K::Float => {
            let value = token.text().replace('_', "");
            value
                .parse::<f64>()
                .map(|_| rua_common::MetaValue::Float(value))
