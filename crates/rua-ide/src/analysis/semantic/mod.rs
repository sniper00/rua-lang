...
pub use crate::analysis::base::FilePosition;
        ...
        let owner = self
            ...
            .min_by_key(|definition| {
                (
                    ...
                    definition.kind() == crate::analysis::hir::DefKind::Chunk,
                )
            })?
            .id();
        Some((
...
fn identifier_at_offset(node: &crate::syntax::SyntaxNode, offset: u32) -> Option<SyntaxToken> {
