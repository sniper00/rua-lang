    ...
    for definition in def_map.definitions() {
            ...
            && let Some(resolution) = db.body_resolution(definition.id())
        {
            for (binding_id, binding) in body.bindings() {
                ...
                let is_used = body.name_refs().any(|(name_ref_id, _nr)| {
                    matches!(
                        ...
                        Some(crate::analysis::hir::LocalResolveResult::Resolved(lid))
            ...
            for (binding_id, binding) in body.bindings() {
                ...
                let lid = crate::analysis::hir::LocalBindingId::new(body.id(), binding_id);
                // Direct writes: `binding = value`
                let has_write = resolution
                    .uses_for(lid)
                    .any(|u| u.kind() == crate::analysis::hir::LocalUseKind::Write);
                ...
                let has_field_write = has_write
                    || body.exprs().any(|(_eid, expr)| {
                        let mut current = match expr {
                            crate::analysis::hir::Expr::Assign { target, .. } => *target,
                            _ => return false,
                        };
...
fn syntax_node_range(node: &crate::syntax::SyntaxNode) -> TextRange {
...
fn add_control_flow_lints(
    ...
    let cfg = crate::analysis::hir::ControlFlowGraph::build(body);
...
fn local_binding_for_expr(
    body: &Body,
    resolution: &BodyResolution,
    expression: crate::analysis::hir::ExprId,
...
fn convert_inference_diagnostic(
    ...
    source_map: &crate::analysis::hir::BodySourceMap,
) -> Option<Diagnostic> {
...
fn inference_source_range(
    _file_id: FileId,
    source: crate::analysis::hir::InferenceSource,
    source_map: &crate::analysis::hir::BodySourceMap,
) -> Option<TextRange> {
    match source {
        crate::analysis::hir::InferenceSource::Expr(expr) => expr_range(expr, source_map),
...
fn expr_range(
    expr: crate::analysis::hir::ExprId,
    source_map: &crate::analysis::hir::BodySourceMap,
) -> Option<TextRange> {
