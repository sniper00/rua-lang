...
/// Semantic types owned by the analysis module.
#[derive(Clone, Debug, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub enum Ty {
...
impl<'a> TypeLoweringContext<'a> {
    ...
    fn lower_path(&self, path: String, args: Vec<Ty>) -> Ty {
        ...
        let builtin = match rua_common::builtin_type(&path) {
            Some(rua_common::BuiltinId::TypeString) if args.is_empty() => Some(Ty::STRING),
            Some(rua_common::BuiltinId::TypeOption) if args.len() == 1 => {
                Some(Ty::Option(Arc::new(args[0].clone())))
            }
            Some(rua_common::BuiltinId::TypeResult) if args.len() == 2 => Some(Ty::Result(
                Arc::new(args[0].clone()),
                Arc::new(args[1].clone()),
            )),
            Some(rua_common::BuiltinId::TypeVec) if args.len() == 1 => {
                Some(Ty::Vec(Arc::new(args[0].clone())))
            }
            Some(rua_common::BuiltinId::TypeHashMap) if args.len() == 2 => Some(Ty::HashMap(
                Arc::new(args[0].clone()),
                Arc::new(args[1].clone()),
            )),
            Some(rua_common::BuiltinId::TypeIter) if args.len() == 1 => {
