...
pub use crate::analysis::diagnostic::{
    ...
    if let Some(callable) = resolution.callable() {
        ...
        if let Some(receiver) = resolution.receiver() {
            params.insert(
                0,
                match receiver {
                    crate::analysis::hir::ReceiverKind::Value => "self",
                    crate::analysis::hir::ReceiverKind::SharedRef => "&self",
                    crate::analysis::hir::ReceiverKind::MutRef => "&mut self",
                }
                .to_string(),
            );
        }
...
fn builtin_type_hover_text(standard_type: &StdType) -> String {
    let keyword = match standard_type.kind() {
        crate::analysis::hir::ItemKind::Struct => "struct",
        crate::analysis::hir::ItemKind::Enum => "enum",
        _ => "type",
    };
...
impl Analysis {
    ...
    fn builtin_type_at(
        ...
        if token.kind() != crate::syntax::SyntaxKind::Ident {
            return None;
        }
    ...
    #[doc(hidden)]
    pub fn query_stats(&self) -> crate::analysis::QueryStats {
        self.db.query_stats()
    }

    #[doc(hidden)]
    pub fn cache_sizes(&self) -> crate::analysis::CacheSizes {
        self.db.cache_sizes()
    }
    ...
    pub fn diagnostics(&self, file_id: FileId) -> Vec<Diagnostic> {
        ...
        let mut diagnostics =
            crate::analysis::diagnostic::fast_diagnostics(&self.db, def_map.clone(), file_id);
        diagnostics.extend(self.annotation_diagnostics(file_id, &def_map, &CfgOptions::default()));
        crate::analysis::diagnostic::normalize_diagnostics(&mut diagnostics);
        diagnostics
    }
    ...
    pub fn diagnostics_in_project(&self, file: ProjectFile) -> Vec<Diagnostic> {
        ...
        let mut diagnostics =
            crate::analysis::diagnostic::fast_diagnostics(&self.db, def_map.clone(), file.file_id);
    ...
    fn annotation_diagnostics(
            ...
            .filter_map(SyntaxAttribute::cast)
        {
            ...
            for (index, item) in core.items.iter().enumerate() {
                ...
                let rua_common::MetaItem::NameValue { name, .. } = item else {
                    diagnostics.push(annotation_diagnostic(
                        file_id,
                        argument_range,
    ...
    ) -> Option<Arc<crate::analysis::semantic::ReferenceIndex>> {
    ...
    fn resolve_call_target(
        &self,
        body: &crate::analysis::hir::Body,
        inference: &std::sync::Arc<crate::analysis::hir::InferenceResult>,
        def_map: &crate::analysis::hir::DefMap,
        member_index: &crate::analysis::hir::MemberIndex,
        expr: &crate::analysis::hir::Expr,
    ) -> Option<(
        crate::analysis::hir::CallableTy,
        Vec<Option<String>>,
        Vec<crate::analysis::hir::ExprId>,
    )> {
        match expr {
            crate::analysis::hir::Expr::Call { callee, args } => {
    ...
    pub fn signature_help(&self, position: ProjectPosition) -> Option<SignatureHelpInfo> {
        ...
        let mut best_expr_id: Option<crate::analysis::hir::ExprId> = None;
        ...
        for definition in query.def_map.definitions().filter(|definition| {
            ...
            for (binding_id, binding) in body.bindings() {
                if binding.name().is_none()
                    ...
                    || matches!(
                        binding.kind(),
                        crate::analysis::hir::BindingKind::Parameter
                            | crate::analysis::hir::BindingKind::ClosureParameter
                            | crate::analysis::hir::BindingKind::SelfParameter
                    )
                {
                    continue;
                }
    ...
    pub fn hover(&self, position: ProjectPosition) -> Option<HoverResult> {
        ...
        if let crate::analysis::hir::LocalResolveResult::Resolved(target) =
    ...
    fn member_hover(
        ...
        if let Some(resolution) = member_index.resolve_field(&receiver_ty, &field_name) {
            ...
            if let crate::analysis::hir::MemberTarget::Definition(id) = resolution.target()
        ...
        if let Some(resolution) = member_index.resolve_method(&receiver_ty, &field_name) {
            ...
            let receiver_str = match resolution.receiver() {
                Some(crate::analysis::hir::ReceiverKind::Value) => "self",
                Some(crate::analysis::hir::ReceiverKind::SharedRef) => "&self",
                Some(crate::analysis::hir::ReceiverKind::MutRef) => "&mut self",
                None => "",
            };
    ...
    fn member_goto_definition(
        ...
        let def_id = match resolution.target() {
            crate::analysis::hir::MemberTarget::Definition(id) => id,
            _ => return None,
        };
    ...
    fn resolve_dot_access(
        ...
        if completion::previous_significant(&token).is_none_or(|t| {
            !matches!(
                t.kind(),
                crate::syntax::SyntaxKind::Dot | crate::syntax::SyntaxKind::QuestionDot
            )
        }) {
            return None;
        }
        let field_name = if token.kind() == crate::syntax::SyntaxKind::Ident {
    ...
    fn definition_at(
        ...
        if let Some((receiver_ty, name, _)) = self.resolve_dot_access(position, &query.def_map) {
            ...
            let crate::analysis::hir::MemberTarget::Definition(def_id) = resolution.target() else {
                return None;
            };
    ...
    fn annotation_parameter_at(
        ...
        let argument = token
            .parent_ancestors()
            .find_map(crate::syntax::ast::MetaItem::cast)?;
    ...
    pub fn call_hierarchy_incoming(&self, item: &CallHierarchyItem) -> Vec<CallHierarchyItem> {
        ...
        for occurrence in index.occurrences(target.id()).iter().filter(|occurrence| {
            occurrence.kind() == crate::analysis::semantic::ReferenceOccurrenceKind::Call
        }) {
            callers
...
fn annotation_diagnostic(
    ...
    Diagnostic::new(
        ...
        range,
        message,
        crate::analysis::diagnostic::DiagnosticOrigin::FastAnalysis,
    )
    .with_code(code)
    .with_source(crate::analysis::diagnostic::DiagnosticSource::Type)
}
...
fn attribute_name_tokens(attribute: &SyntaxAttribute) -> Vec<crate::syntax::SyntaxToken> {
...
fn item_hover_text(
    ...
    if definition.kind() == DefKind::Variant
        && let crate::analysis::hir::ItemSignature::Variant(signature) = definition.signature()
    {
        return match signature.kind() {
            crate::analysis::hir::VariantKind::Unit => definition.name().to_string(),
            crate::analysis::hir::VariantKind::Tuple => format!(
                "{}({})",
                definition.name(),
                signature
                    .tuple_types()
            ...
            crate::analysis::hir::VariantKind::Struct => {
                let fields = def_map
...
fn push_type_hint_parts(
    ...
    match ty {
        Ty::Primitive(primitive) => {
            if matches!(primitive, crate::analysis::hir::PrimitiveTy::String) {
...
fn standard_function_context(function: &StdFunction) -> String {
    rua_common::module_path_from_relative_file(Path::new(function.source_path()))
        .ok()
        .flatten()
...
fn standard_context(source_path: &str, owner: &str) -> String {
    let module = rua_common::module_path_from_relative_file(Path::new(source_path))
        .ok()
        .flatten()
        .filter(|segments| !segments.is_empty())
...
fn resolve_callee_param_names(
    body: &crate::analysis::hir::Body,
    def_map: &crate::analysis::hir::DefMap,
    callee: crate::analysis::hir::ExprId,
) -> Vec<Option<String>> {
    let Some(crate::analysis::hir::Expr::Path(path)) = body.expr(callee) else {
        return vec![];
    };
    let [nr] = &path[..] else {
        return vec![];
...
mod tests {
    ...
    use crate::analysis::vfs::{Change, FileId, FileKind, SourceRootId, SourceRootKind};
    ...
    fn declaration_files_reject_executable_bodies_and_top_level_statements() {
        ...
        let invalid = diagnostics
            .iter()
            .filter(|diagnostic| {
                diagnostic.code() == Some(rua_common::DiagnosticCode::NameInvalidDeclaration)
            })
            .collect::<Vec<_>>();
