...
空 literal 应通过 annotation 提供类型，例如
`let empty: HashMap<String, i64> = #{};`。codegen 按 entry 数预分配 map 的 hash
capacity，再按源码顺序插入，以保持重复 key 的覆盖顺序与 entry 副作用顺序。
...
declaration root，并使用相同相对路径规则。旧
`workspace.library` 与 `library_mounts` 只用于 declaration-only 输入和非约定映射。
...
identity-resolved 依赖，随后定义并初始化当前 module，最后返回 module table。
...
依赖环会在编译期拒绝；需要循环依赖的程序应重构依赖方向或使用 bundle。
